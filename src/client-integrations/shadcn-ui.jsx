      ...
      <CarouselContent>
        {items.map((item) => (
          <CarouselItem
            ...
            key={item.id || `carousel-item-${Math.random().toString(36).substr(2, 9)}`}
          >
            {item.content}
          </CarouselItem>
        ))}
...
function CustomContextMenu({ triggerLabel, items }) {
      ...
      <Fragment key={item.label}>
        {item.subItems ? (
          <ContextMenuSub>
            ...
            <ContextMenuSubContent>
              <ContextMenuGroup>{renderMenuItems(item.subItems)}</ContextMenuGroup>
            </ContextMenuSubContent>
          </ContextMenuSub>
        ) : (
          <ContextMenuItem onSelect={item.onSelect} key={item.label} disabled={item.disabled}>
            {item.label}
  ...
  return (
    <ContextMenu>
      <ContextMenuTrigger className="h-full w-full">{triggerLabel}</ContextMenuTrigger>
      <ContextMenuContent>
      ...
      <Fragment key={item.label}>
        {item.subItems ? (
          <DropdownMenuSub>
            ...
            <DropdownMenuPortal>
              <DropdownMenuSubContent>{renderMenuItems(item.subItems)}</DropdownMenuSubContent>
            </DropdownMenuPortal>
          </DropdownMenuSub>
        ) : (
          <DropdownMenuItem>
          ...
          <MenubarContent>
            {menu.items.map((item) => {
              switch (item.type) {
                ...
                case 'item':
                  return (
                    <MenubarItem key={item.label} disabled={item.disabled} onSelect={item.onSelect}>
                      {item.label}
            ...
            <NavigationMenuContent>
              <ul className="grid gap-3 p-6 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr]">
                {section.items.map((item) => (
                  <li key={item.href} className={`row-span-${item.rowSpan ?? 1}`}>
