...
function getAuthSecret() {
  const secret =
    process.env.AUTH_SECRET || process.env.OTP_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY
...
function sign(input: string) {
  return base64UrlEncode(createHmac('sha256', getAuthSecret()).update(input).digest())
}

function verifySignedJwt(token: string | null, secret: string) {
  try {
    ...
    const expected = base64UrlEncode(createHmac('sha256', secret).update(unsigned).digest())
...
export function verifyPassword(password: string, storedHash: string) {
  ...
  const actual = pbkdf2Sync(password, salt, Number(iterations), HASH_KEY_LENGTH, HASH_DIGEST)
  const expectedBuffer = Buffer.from(expected, 'hex')

  return actual.length === expectedBuffer.length && timingSafeEqual(actual, expectedBuffer)
}

export function verifyLegacyBase64Password(password: string, storedPassword: string) {
...
export async function requireAuth(
  ...
  error: Response | null
  user: { sub: string; email: string | null; role: string } | null
}> {
  ...
  const authClient = createSupabaseAuthClient(request)
  ...
  const claims = {
    sub: authUser.id,
    email: (authUser.email ?? profile.email ?? null) as string | null,
