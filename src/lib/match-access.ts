import { getCanonicalRole } from '@/lib/auth-server'
import type { SupabaseClient } from '@supabase/supabase-js'

export async function loadAcceptedPatientDonorMatch(
  supabase: SupabaseClient,
  matchId: string,
  auth: { user: { sub: string; role: string } }
) {
  if (!matchId) {
    return {
  ...
  if (match.match_status !== 'Accepted') {
    return {
      error: Response.json(
        { error: 'Chat and tracking unlock after donor acceptance' },
        { status: 403 }
      ),
    }
  }
