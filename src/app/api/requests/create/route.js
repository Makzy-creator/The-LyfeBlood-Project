...
import { getBearerToken, getCanonicalRole } from "@/app/api/utils/auth";
...
/**
 * Normalize a blood_type_needed value (string or array) into a comma-joined
 * string, deduplicating while preserving order. Returns null when empty so the
  ...
  try {
    ...
    if (!profile) {
      return Response.json(
        {
          error:
            "Your account profile is missing. Please complete your profile and try again.",
        },
        { status: 409 },
      );
    }
    ...
    try {
      ...
      console.error(
        "[POST /api/requests/create] notification side effect failed",
        error,
      );
    ...
    try {
      ...
      console.error(
        "[POST /api/requests/create] matching side effect failed",
        error,
      );
