...
export default function RequestDetailsPage() {
  ...
  useEffect(() => {
    ...
    let isActive = true
    // eslint-disable-next-line react-hooks/set-state-in-effect -- reset loading/error states before fetch
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, paddingBottom: '80px' }}>
          ...
          <div style={{ padding: '24px 12px' }}>
            <Section title="Unable to load request" icon={AlertTriangle}>
              ...
              <SecondaryButton onClick={() => router.push(homeRoute)} icon={ChevronLeft}>
                Back to Dashboard
              </SecondaryButton>
            </Section>
          </div>
        ...
        >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  ...
                  <p style={{ margin: 0, fontSize: '12px', color: '#6B6B6B' }}>{request.ward}</p>
                </div>
              </div>
          ...
          <Section title="Verification & Status" icon={ShieldCheck}>
            ...
            <p style={{ margin: 0, fontSize: '12px', color: '#4A4A4A', lineHeight: '1.5' }}>
              Donors should only proceed after confirming this request with hospital staff. Hospital
              staff should update the status only after verification at the facility.
            </p>
            {statusError && (
            ...
            <Section title="Donor Coordination" icon={Navigation}>
              ...
              {acceptedMatches.map((match) => (
                ...
                >
                  ...
                  <PrimaryButton
                    onClick={() => router.push(`/matches/${match.id}/tracking`)}
                    icon={Navigation}
                  >
                    Track Donor
                  </PrimaryButton>
          ...
          <Section title="Privacy & Consent" icon={ShieldCheck}>
            ...
            >
              ...
              <span style={{ fontSize: '12px', color: '#4A4A4A', lineHeight: '1.5' }}>
                I understand this request may contain sensitive medical context and should only be
                used for donation coordination with authorized hospital staff.
              </span>
            </label>
            {consented && (
              <p style={{ margin: 0, fontSize: '12px', color: '#1E8449', fontWeight: '700' }}>
