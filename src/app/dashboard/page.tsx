...
function PatientRequestSheet({ onClose, onSubmit, isSubmitting, submitError }) {
  ...
  const handleSubmit = (event) => {
    ...
    onSubmit({
      ...
      scheduledFor:
        form.requestType === 'Scheduled' ? new Date(form.scheduledFor).toISOString() : null,
    })
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <label style={{ fontSize: '13px', fontWeight: '700', color: '#1A1A1A' }}>Note</label>
          <textarea
...
export default function DashboardPage() {
    ...
    try {
      ...
      setMatchingState((current) => ({
        ...
        matches: current.matches.map((match) =>
          current.selectedIds.includes(match.id) ? { ...match, match_status: 'Alerted' } : match
        ),
      }))
    } catch (error) {
      setMatchingState((current) => ({
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars -- used in JSX; ESLint can't see past early return
      ...
      >
        ...
        >
          ...
          >
            ...
            {currentUser.bloodGroup && <BloodGroupTag group={currentUser.bloodGroup} />}
        ...
        {(matchingState.loading || matchingState.request) && (
          ...
          >
            <div>
              <h2
                style={{ margin: '0 0 3px', fontSize: '15px', fontWeight: '800', color: '#1A1A1A' }}
              >
                Eligible Donors
              </h2>
        ...
        {unreadCount > 0 && (
          ...
          >
            ...
            >
              {unreadCount} unread notification{unreadCount > 1 ? 's' : ''} — tap the bell to clear
            </span>
          </div>
        )}
