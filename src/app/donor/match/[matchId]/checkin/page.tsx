...
import { Shield, CheckCircle2, Building2, AlertTriangle } from 'lucide-react'
...
function readOtpFromStorage(matchId) {
  if (typeof window === 'undefined') return { otp: '', expiresAt: null, ttl: null }
  const prefix = `lyfeblood.match.${matchId}`
  return {
    otp: window.sessionStorage.getItem(`${prefix}.otp`) ?? '',
    expiresAt: window.sessionStorage.getItem(`${prefix}.expires_at`),
    ttl: Number.parseInt(window.sessionStorage.getItem(`${prefix}.ttl_seconds`) ?? '', 10),
  }
}
...
export default function CheckInPage() {
  ...
  const [otp, _setOtp] = useState(() => readOtpFromStorage(matchId).otp)
  const [expiresAt, _setExpiresAt] = useState(() => readOtpFromStorage(matchId).expiresAt)
  const [durationSeconds, _setDurationSeconds] = useState(() => {
    const { ttl } = readOtpFromStorage(matchId)
    return Number.isFinite(ttl) && ttl > 0 ? ttl : OTP_DURATION_SECONDS
  })
  const [match, setMatch] = useState(null)
  const [secondsLeft, setSecondsLeft] = useState(OTP_DURATION_SECONDS)
  const [copied, setCopied] = useState(false)
  const [loadError, setLoadError] = useState(() => {
    const { otp: storedOtp, expiresAt: storedExpiresAt } = readOtpFromStorage(matchId)
    return !storedOtp || !storedExpiresAt
      ? 'OTP details are unavailable. Please accept the match again.'
      : null
  })
  ...
  useEffect(() => {
    ...
    const updateSecondsLeft = () => {
      setSecondsLeft(Math.max(0, Math.ceil((Date.parse(expiresAt) - Date.now()) / 1000)))
    }
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Shield size={18} color="#1E8449" style={{ flexShrink: 0, marginTop: '1px' }} />
          <p
            style={{
              fontSize: '13px',
              color: '#1E8449',
          ...
          >
            <strong>Present this secure code token</strong> to the hospital laboratory manager upon
            arrival. Do not share this code with anyone else.
          </p>
        </div>
        ...
        >
          ...
          {isExpired && (
            ...
            >
              ...
              <div>
                ...
                >
                  Your 15-minute verification window has lapsed. Please contact the hospital
                  directly or return to dashboard and re-accept the request.
                </p>
              </div>
            </div>
          )}
          ...
          <div style={{ paddingBottom: '32px' }}>
            ...
            <PrimaryButton onClick={() => {}} disabled icon={CheckCircle2}>
            ...
            >
              Tap only after you have physically arrived at the hospital laboratory.
            </p>
          </div>
        </div>
      </div>
