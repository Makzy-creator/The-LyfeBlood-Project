...
async function apiGetRequests(params = {}) {
...
var MATCH_MOCK = {
  ...
  distanceKm: 4.2,
  unitsNeeded: 3,
  urgencyNote: "Post-operative haemorrhage. Critical \u2014 patient is O-, universal donor needed.",
  patientCode: "FMC-AE-2077",
  tier: "sos"
};
var AppContext = (0, import_react.createContext)(null);
var AUTH_STORAGE_KEY = "lyfeblood.auth.user";
...
function normalizeRequestStatus(status) {
  switch (status) {
    ...
    case "Arrived":
    case "Arrived At Lab":
    case REQUEST_STATUS.ARRIVED_AT_LAB:
      return REQUEST_STATUS.ARRIVED_AT_LAB;
    case "Completed":
...
function buildRequestNotifications(currentUser, bloodRequests) {
  ...
  return activeRequests.filter((request) => {
    ...
    return !currentUser.bloodGroup || request.bloodGroup === currentUser.bloodGroup;
  }).map((request) => {
    const status = statusLabel(request.status);
    const prefix = role === "donor" ? `${request.bloodGroup ?? "Blood"} needed` : `${request.bloodGroup ?? "Blood"} request ${status}`;
    return {
      id: `${role}-${request.id}-${request.status}`,
      type: request.tier === "sos" ? "sos_alert" : "request_update",
      message: `${prefix} at ${request.hospitalName}.`,
...
function AppProvider({ children }) {
  const [currentUser, setCurrentUser] = (0, import_react.useState)(getInitialUser);
  const [isAuthenticated, setIsAuthenticated] = (0, import_react.useState)(() => {
    return !!getInitialUser();
  });
  const [bloodRequests, setBloodRequests] = (0, import_react.useState)([]);
  const [readNotificationIds, setReadNotificationIds] = (0, import_react.useState)(() => /* @__PURE__ */ new Set());
  const [activeNav, setActiveNav] = (0, import_react.useState)("home");
  const [donorAvailable, setDonorAvailable] = (0, import_react.useState)(true);
  const [incomingMatchAlert, setIncomingMatchAlert] = (0, import_react.useState)(null);
  const refreshBloodRequests = (0, import_react.useCallback)(async () => {
  ...
  const updateCurrentUser = (0, import_react.useCallback)((nextUser) => {
  ...
  const toggleDonorAvailable = (0, import_react.useCallback)(() => {
    setDonorAvailable((v) => !v);
  }, []);
  const triggerMatchAlert = (0, import_react.useCallback)((matchData) => {
    if (matchData) setIncomingMatchAlert(matchData);
  }, []);
  const dismissMatchAlert = (0, import_react.useCallback)(() => {
    setIncomingMatchAlert(null);
  }, []);
  const updateRequestStatus = (0, import_react.useCallback)((requestId, newStatus) => {
    setBloodRequests(
      (prev) => prev.map(
  ...
  const addRequest = (0, import_react.useCallback)(async (newRequest) => {
  ...
  const notifications = (0, import_react.useMemo)(() => {
    return buildRequestNotifications(currentUser, bloodRequests).map((notification) => ({
      ...notification,
      isRead: readNotificationIds.has(notification.id)
    }));
  }, [bloodRequests, currentUser, readNotificationIds]);
  const markAllNotificationsRead = (0, import_react.useCallback)(() => {
