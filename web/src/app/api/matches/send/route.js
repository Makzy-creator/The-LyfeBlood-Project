...
function deliveryForRequest(bloodRequest) {
  ...
  const deliverAt = new Date(bloodRequest.scheduled_for).getTime() - 2 * 86_400_000;
...
export async function POST(request) {
  try {
    ...
    const { data: bloodRequest, error: requestError } = await supabase
      .from("blood_requests")
      .select("id, hospital_name, blood_type_needed, requested_by, hospital_id, request_type, scheduled_for")
      .eq("id", requestId)
      .maybeSingle();
